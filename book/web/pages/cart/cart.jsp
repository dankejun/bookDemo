	...
	<script type="text/javascript">
		$(function () {
			...
			$(".updateCount").change(function () {
				...
				if (confirm("确定要修改【" + itemName + "】的数量为" + count + "吗")) {
	...
	<div id="main">
			...
			<c:if test="${not empty sessionScope.cart.items}">
				<c:forEach items="${sessionScope.cart.items}" var="entry">
					<tr>
						...
						<td>
							<input class="updateCount" bookId="${entry.value.id}" style="width:
							78px"
								   type="text"
		...
		<c:if test="${not empty sessionScope.cart.items}">
			...
			<span class="cart_span">购物车中共有<span class="b_count">${sessionScope.cart.totalCount}</span>件商品
				...
				<span
						class="cart_span"><a id="clearCart" href="cartServlet?action=clearCart">
					清空购物车</a></span>
